...
if command_exists browser-mcp-server; then
    INSTALLED_VERSION=$(npm list -g browser-mcp-server --depth=0 2>/dev/null | grep browser-mcp-server | awk '{print $2}' | tr -d '@')
    ...
    if [ "$response" = "y" ] || [ "$response" = "Y" ]; then
        ...
        npm uninstall -g browser-mcp-server 2>&1 | grep -v "npm WARN"
    else
...
if [ "$SKIP_NPM_INSTALL" != "true" ]; then
    ...
    npm install -g browser-mcp-server 2>&1 | tail -5
