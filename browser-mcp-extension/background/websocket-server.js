  ...
  async start() {
    ...
    for (let port = this.options.port; port < this.options.port + 10; port++) {
      try {
        ...
        console.log(`[WebSocketServer] Port ${port} busy, trying next...`);
        continue;
      }
    }

    throw new Error('Failed to start WebSocket server: No available ports');
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      chrome.sockets.tcpServer.create({}, (createInfo) => {
        ...
        chrome.sockets.tcpServer.listen(
          ...
          (result) => {
            if (result < 0) {
              reject(new Error(`Failed to listen: ${result}`));
              return;
            }
